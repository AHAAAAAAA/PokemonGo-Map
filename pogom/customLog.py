...
def pokeData(pokemon,lat,lng,despawntime):
    if args.datatext:
        pokemon_name = get_pokemon_name(pokemon).lower()
        data = True
        if data:
            ...
            with open(datafile, "a") as myfile:
                myfile.write("%s,%s,%s,%s,%s,%s,%s\n" % (pokemon_name.encode('utf-8').title(), lat, lng, spawntime.strftime("%H:%M"),
                             spawntime.strftime("%a"), spawntime.strftime("%b %d,%Y"), desptime.strftime("%H:%M")))
