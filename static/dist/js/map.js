...
function initMap() {
  ...
  searchMarkers = $.map(originalLocations, function (loc) {
    return createSearchMarker(loc.lat, loc.lon, loc.name);
  });
...
function createSearchMarker(lat, lng, name) {
  ...
  google.maps.event.addListener(marker, 'dragend', function () {
    ...
    changeSearchLocation(newLocation.lat(), newLocation.lng(), oldLocation.lat(), oldLocation.lng()).done(function () {
      buildSearchList();
      oldLocation = null;
    }).fail(function () {
      if (oldLocation) {
...
function removeLocation(lat, lng) {
  removeSearchLocation(lat, lng).done(function () {
    $.each($.grep(searchMarkers, function (searchMarker) {
      return searchMarker.getPosition().lat() == lat && searchMarker.getPosition().lng() == lng;
