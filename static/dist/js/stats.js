		...
		$.each(map_data.pokemons, function (key, value) {
			...
			for (var i = 0; i < pkmnCount.length; i++) {
				if (pkmnCount[i] != null && pkmnCount[i].Count > 0) {
					pkmnListString += "<tr><td><img src=\"/static/icons/" + pkmnCount[i].ID + ".png\" /></td><td><a href='http://pokemondb.net/pokedex/" + pkmnCount[i].Name + "' target='_blank' title='View in Pokedex' style=\"color: black;\">" + pkmnCount[i].Name + "</a></td><td>" + pkmnCount[i].Count + "</td><td>" + Math.round(pkmnCount[i].Count * 100 / pkmnTotal * 10) / 10 + "%</td></tr>";
				}
			}
