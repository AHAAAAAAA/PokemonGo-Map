(function() {
    ...
    let $selectExclude = $("#exclude-pokemon");
    ...
    window['initMap'] = function() {  // jshint ignore:line
        ...
        let style_dark = new google.maps.StyledMapType(darkStyle, {name: "Dark"});
        map.mapTypes.set('dark_style', style_dark);

        let style_light2 = new google.maps.StyledMapType(light2Style, {name: "Light2"});
        map.mapTypes.set('style_light2', style_light2);

        let style_pgo = new google.maps.StyledMapType(pGoStyle, {name: "PokemonGo"});
    ...
    function pokemonLabel(name, disappear_time, id, latitude, longitude) {
        let disappear_date = new Date(disappear_time);
        let pad = function (number) { return number <= 99 ? ("0" + number).slice(-2) : number; };
    ...
    function gymLabel(team_name, team_id, gym_points) {
        let gym_color = ["0, 0, 0, .4", "74, 138, 202, .6", "240, 68, 58, .6", "254, 217, 40, .6"];
        let str;
        if (team_name === 0) {
            str = `<div><center>
                <div>
    ...
    // Dicts
    let map_pokemons = {}; // Pokemon
    let map_gyms = {}; // Gyms
    let map_pokestops = {}; // Pokestops
    let gym_types = ["Uncontested", "Mystic", "Valor", "Instinct"];
    ...
    function setupPokestopMarker(item) {
        let imagename = item.lure_expiration ? "PstopLured" : "Pstop";
        let marker = new google.maps.Marker({
            position: {
                lat: item.latitude,
                lng: item.longitude
    ...
    let updateLabelDiffTime = function() {
        $('.label-countdown').each(function(index, element) {
            let disappearsAt = new Date(parseInt(element.getAttribute("disappears-at")));
            let now = new Date();

            let difference = Math.abs(disappearsAt - now);
            let hours = Math.floor(difference / 36e5);
            let minutes = Math.floor((difference - (hours * 36e5)) / 6e4);
            let seconds = Math.floor((difference - (hours * 36e5) - (minutes * 6e4)) / 1e3);
